...
struct PollTimeline: View {
        ...
        VStack(alignment: .leading, spacing: 16.0) {
            ...
            VStack(spacing: 24.0) {
                ForEach(viewModel.viewState.poll.answerOptions) { answerOption in
                    Button {
                        viewModel.send(viewAction: .selectAnswerOption(answerOption))
                    } label: {
                        ...
                        VStack(alignment: .leading, spacing: 12.0) {
                            HStack(alignment: .top, spacing: 8.0) {
                                ...
                                if answerOption.winner {
                                    Image(uiImage: Asset.Images.pollWinnerIcon.image)
                                }
                                
                                Text(VectorL10n.pollTimelineVotesCount(Int(answerOption.count)))
                                    .font(theme.fonts.footnote)
                                    .foregroundColor(answerOption.winner ? theme.colors.accent : theme.colors.secondaryContent)
                            }
                        ...
                        .padding(.bottom, 8.0)
                        .clipShape(rect)
                        .overlay(rect.stroke(borderColorForAnswerOption(answerOption) , lineWidth: 1.0))
                        .accentColor((answerOption.selected ? theme.colors.accent : theme.colors.quarterlyContent))
                    }
                }
            }
            
            Text(VectorL10n.pollTimelineTotalVotesCount(Int(viewModel.viewState.poll.totalAnswerCount)))
                .font(theme.fonts.footnote)
    ...
    private func borderColorForAnswerOption(_ answerOption: PollTimelineAnswerOption) -> Color {
